...
$fileName = 'cap_report_'.str_replace('/', '.', $week_start).'-'.str_replace('/', '.', $today).'.xls';
...
//    $activeSheet->setTitle($crmName);
    ...
    $dbApi = DBApi::getInstance();
    $all_crm = $dbApi->getTrialCampaignResult($fromDate, $toDate);
    $row = 0;
    for ($crm_id = 0; $crm_id < sizeof($all_crm); $crm_id++) {
        ...
        $activeSheet->setCellValue('B'.$row, 'Sales');
        $activeSheet->setCellValue('C'.$row, 'Approval Rate');
        $activeSheet->setCellValue('D'.$row, 'Rebill %');
