...
$fileName = 'rebill_'.$crmName.'_'.str_replace('/', '.', $fromDate).'-'.str_replace('/', '.', $toDate).'.xls';
...
//    $activeSheet->setTitle($crmName);
    ...
    $activeSheet->setCellValue('B1', 'Sales');
    $activeSheet->setCellValue('C1', 'Approval Rate');
    $activeSheet->setCellValue('D1', 'Rebill %');
    cellColor($activeSheet, 'A1:D1', 'FFC7CE', '9C0006', true);

    $dbApi = DBApi::getInstance();
    $campaignList = $dbApi->getTrialCampaignResultById($crmID, $fromDate, $toDate);
    ...
    for ($i = 0; $i < sizeof($campaignList); $i++)
    {
        ...
        $color = $campaign[4] < 65.0 ? 'FF0000': ($campaign[4] < 80.0 ? 'FFFF00': '00FF00');
        ...
        for ($j = 0; $j < sizeof($affiliates); $j++) {
            ...
            $color = $affiliate[4] < 65.0 ? 'FF0000': ($affiliate[4] < 80.0 ? 'FFFF00': '00FF00');
            cellColor($activeSheet, 'D'.$row, $color);
            ...
            for ($k = 0; $k < sizeof($sub_affiliates); $k++) {
                ...
                $color = $sub_affiliate[4] < 65.0 ? 'FF0000': ($sub_affiliate[4] < 80.0 ? 'FFFF00': '00FF00');
                cellColor($activeSheet, 'D'.$row, $color);
            }
        }
    }
