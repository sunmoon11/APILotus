...
$fileName = $exportType.'_'.$crmName.'_'.$fromDate.'_'.$toDate.'.xls';
...
$objWriter = PHPExcel_IOFactory::createWriter(makeExportData($objPHPExcel, $userToken, $crmID, $crmName, $fromDate, $toDate), 'Excel5');
$objWriter->save('php://output');

exit;

function makeExportData($phpExcel, $userToken, $crmID, $crmName, $fromDate, $toDate)
{
    ...
    $activeSheet->setCellValue('B1', $crmName);
    $activeSheet->setCellValue('A2', 'From Date');
    $activeSheet->setCellValue('B2', $fromDate);
    $activeSheet->setCellValue('A3', 'To Date');
    $activeSheet->setCellValue('B3', $toDate);

    $dbApi = DBApi::getInstance();
    $campaignList = $dbApi->GetCampaignList4QuickExport($crmID, $userToken);
    $row = 3;
    for ($j = 0; $j < sizeof($campaignList); $j ++) 
    {
        ...
        // set affiliate sum
        $affiliateList = $dbApi->GetAffiliateList4QuickExport($crmID, $campaignList[$j][0], $userToken);
        ...
        for ($k = 0; $k < sizeof($affiliateList); $k ++) 
        {
            ...
            $subAffiliateList = $dbApi->GetSubAffiliateList4QuickExport($crmID, $campaignList[$j][0], $affiliateList[$k][0], $userToken);
