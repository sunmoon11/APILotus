...
//            else
    ...
    public function getCrmResultForAlert13()
    {
        ...
        try {
            ...
            $fromDate = $this->getDateOfCurrentMonday();
            $toDate = date('m/d/Y');
    ...
    public function getCrmResultForAlert()
    {
        if (!$this->checkConnection())
            return 'error';
        ...
        try {
            ...
            if ($crm_count > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $arrayCrm[] = array(
                        "crm_id" => $row['crm_id'],
                        "crm_name" => $this->getCrmName($row['crm_id']),
                        "step1" => $row['step1'],
                        "step2" => $row['step2'],
                        "takerate" => $takeRate,
                        "tablet" => $row['tablet'],
                        "tablet_takerate" => $tabletTakeRate,
                        "goal" => $row['goal'],
    ...
    public function getAllOffers()
    {
        ...
        try {
            ...
            if ($count > 0) {
                while($row = mysqli_fetch_assoc($result))
                    $ret[] = array($row['id'], $row['name'], $row['crm_id'], $row['campaign_ids'], $row['label_ids'], $row['offer_type'], $row['s1_payout'], $row['s2_payout']);
            }
    ...
    public function getOffersByAffiliateID($affiliate_id)
    {
        if (!$this->checkConnection())
            return null;
